...
import { assignModuleToRole, getRoles } from "../../api/roles";
...
const form = ref({ name: "", url: "", parent_id: "", role_ids: [] });
const formError = ref("");
...
async function load() {
    loading.value = true;
    try {
        const [modulesResponse, rolesResponse] = await Promise.all([getModules(), getRoles()]);
...
function openCreate() {
    ...
    form.value = { name: "", url: "", parent_id: "", role_ids: [] };
    formError.value = "";
...
async function onSubmit() {
    ...
    try {
        ...
        if (editingModule.value) {
            const { data } = await updateModule(editingModule.value.id_module, payload);
        ...
        if (form.value.role_ids.length > 0) {
            await Promise.all(form.value.role_ids.map(roleId => assignModuleToRole(roleId, moduleId)));
        }
    ...
    <div>
        ...
        <div class="mt-6">
            <DataTable
                ...
                row-key="id_module"
                :loading="loading"
                empty-message="Todavía no hay módulos registrados."
            >
        ...
        >
            <form class="p-6" novalidate @submit.prevent="onSubmit">
                ...
                <div class="mt-4 flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label for="module-url" class="text-sm font-medium"
                            >URL <span class="font-normal text-ink-muted">(solo nodos hoja)</span></label
                        >
                        <input
                            id="module-url"
                            v-model="form.url"
                            placeholder="/ejemplo"
                            class="rounded-lg border border-border bg-bg px-3 py-2 text-sm font-mono outline-none focus-visible:ring-2 focus-visible:ring-accent"
                        />
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <span class="text-sm font-medium">Roles con acceso</span>
                        <p class="text-xs text-ink-muted">
                            Un módulo sin roles asignados no aparece en el menú de nadie.
                        </p>
                        <div class="flex max-h-32 flex-col gap-1.5 overflow-y-auto rounded-lg border border-border p-2">
                            <label v-for="role in roles" :key="role.id_role" class="flex items-center gap-2 text-sm">
                                <input
                                    type="checkbox"
                                    :value="role.id_role"
                                    v-model="form.role_ids"
                                    class="rounded border-border text-accent focus-visible:ring-2 focus-visible:ring-accent"
                                />
                                {{ role.name_role }}
                            </label>
                            <p v-if="roles.length === 0" class="text-sm text-ink-muted">
                ...
                <p v-if="formError" class="mt-4 rounded-lg bg-danger/10 px-3 py-2 text-sm text-danger" role="alert">
                    {{ formError }}
                </p>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    ...
                    <button
                        type="submit"
                        :disabled="submitting"
                        class="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-accent-ink transition active:scale-[0.98] disabled:opacity-60"
                    >
                        Guardar
                    </button>
                </div>
